""" Classe de maintenance des données explicatives SHAP par rapport à un modelet un jeu de données spécifiques"""
...
def main():
    ...
    # Chargement de Modele de ML utilisé
    xgboost_model = fGetRessources(os.path.join("data", "cleaned", "xgboost_model.pkl"))

    # Calculs SHAP
    explainer = shap.explainers.Permutation(xgboost_model.predict_proba, df)
    shap_values = explainer(df)
    
    #Sauvegarde des données pour réutilisation dans le dashboard
    path_file_data = os.path.join("data", "cleaned", "shap_explainer.pkl")
    pickle.dump(explainer, open(path_file_data, "wb"))

    path_file_data = os.path.join("data", "cleaned", "shap_values.pkl")
    pickle.dump(shap_values, open(path_file_data, "wb"))
